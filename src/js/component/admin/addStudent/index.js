 ...
 * */
let name= 'excelFile';
let data = {method:"insertStudentByFile"};
...
class AddStudent extends React.Component{
    ...
    handleChange(info){
        const {addStudentViaFileFunc} = this.props;
        addStudentViaFileFunc(info);
    }

    render(){
        const {uploadFileUrlOfAddStudentURL,studentAdded} = this.props;
        const props = {
            action: uploadFileUrlOfAddStudentURL,
            name:name,
            data:data,
        ...
        return(
            <Row style={{minHeight:500}}>
                <Row className="markdown"><h2>导入学生</h2></Row>
                ...
                <Table dataSource={studentAdded} columns={columns}/>
            </Row>
        )
    }
}
